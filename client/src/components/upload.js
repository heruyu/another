...
function Upload() {
  // onclick wchodzi plik
  // const [UFile, setUFile] = useState();
  const [Message, setMessage] = useState();
  const handleUpload = () => {
    const fileInput = document.getElementById("fileInput");
    const filePath = { path: fileInput.files[0]?.path, method: "copy" };
    if (filePath.path) {
      axios
  ...
  return (
    <div className="inline-block ring-blue">
      ...
      <div className="text-xs font-medium uppercase leading-normal text-white">
        Response: {Message}
      </div>
    </div>
  );
