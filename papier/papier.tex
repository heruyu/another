...
Celem niniejszej pracy jest omówienie projektu systemu, który upraszcza użytkownikowi proces gromadzenia i sortowania informacji. Skupi się ona na przedstawieniu koncepcji aplikacji oraz  opisie poszczególnych implementacji, w tym modułu odpowiedzialnego za obsługę artykułów.\par
...
\subsection*{Opisy funkcjonalne modułów}
...
\end{enumerate}\par
Interfejs aplikacji został opracowany przy użyciu języka JavaScript z biblioteką React, znaną z wydajnej architektury opartej na komponentach i płynnych możliwości tworzenia interfejsu użytkownika. W implementacji serwera został wykorzystany język Python, w połączeniu z frameworkiem Flask. Opis poszczególnych elementów znajduje się poniżej.\par
...
\end{itemize}
\subsection{Status}
Ze względu na ograniczenia czasowe, implementacja obejmuje następujące elementy określone w rozdziale 2. Odchylenia od założeń przedstawionych w tym rozdziale zostały opisane w rozdziale 3.3, a poniżej wydnieje lista częściowo lub w pełni zawartych elementów w zależności od modułu. 
\begin{enumerate}
	...
	\end{itemize}
	Pełen opis specyfikacji zależnej od rodzaju jest przedstawiony w rozdziale 3.4.
\end{enumerate}
